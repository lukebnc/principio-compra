...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO orders (user_id, product_id, quantity, total_price, payment_method, payment_address, status) VALUES (?, ?, ?, ?, ?, ?, 'pending')");
        ...
        $_SESSION['success'] = "¡Orden realizada con éxito! Por favor completa tu pago con Monero (XMR).";
        redirect('orders.php');
        
    } catch (PDOException $e) {
        // Rollback on error
        $conn->rollBack();
        $_SESSION['error'] = "Error al procesar la orden. Por favor intenta de nuevo.";
        redirect('cart.php');
    }
}
